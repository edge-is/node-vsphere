...
Client.prototype.getTasks = function (){
  ...
  var propertyCollector = self.serviceContent.propertyCollector;
  self.runCommand('RetrieveServiceContent', {
    _this : 'ServiceInstance'
   }).once('result', function (res){
    var propertySpec = {
       attributes: {'xsi:type': 'PropertySpec'},
       type: 'Task',
       all: false
    ...
    self.runCommand('RetrievePropertiesEx', {
     ...
     .once('result', function(result){
       ...
       if (!result.returnval) return;

       if (!result.returnval.objects) return console.log('no objects');

       var tasks = [];

       if (!Array.isArray(result.returnval.objects)){
         tasks = [ result.returnval.objects ];
       }else{
         tasks = result.returnval.objects;
       }
       var allTasks = [];
       ...
       async.each(tasks, function (task, next){
         self._taskInfo(task, function (err, res){
           if(err) return next();
           allTasks.push(res);
           next();
         });
         ...
         return emitter.emit('result', allTasks);
       });
     }).once('error', function (err){
       return emitter.emitt('error', err);
     })
  });
...
Client.prototype._taskInfo = function(task, callback){
  var self = this;
   self.waitForValues( task.obj , [
     'info.state','info.error', 'info.name',
     'info.startTime', 'info.task', 'info.entityName',
     'info.state', 'info.reason'], 'state', ['success','error'])
   .once('result', function( result ) {
